// Shared client for the Network-based fixed-distance SlopeSection Backend API. Used by the 4 slope
// visualization screens (MountainDetailView, CompareCourseView, MountainDetailView2,
// MobileMountainDetailView) so all of them use the exact same window size and color rule,
// replacing the old per-screen groupCoordinates(5/7/12) + calculateSlope().
//
// estimatedSlopePercent is a DEM-derived ESTIMATED slope, not a measured one.
...
// database happened to assign on import, so it differs between a fresh rebuild and a long-lived
// database. This module MUST NOT hardcode those values. Name-only compatibility between the
// `course` DB table's courseName ("무악동", no "구간" suffix -- verified against the real DB) and the Trail GeoJSON
 ...
 * `courseName` may come from either the Trail GeoJSON's own PMNTN_NM (exact match) or the shorter `course` DB table name (resolved through the alias
...
// Thresholds chosen from the real 20m estimatedSlopePercent distribution: |slope| p90 is roughly 40%, so this flags only the steepest ~10% of
...
// are not the same quantity, so their thresholds are not comparable.
