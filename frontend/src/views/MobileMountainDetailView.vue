<template>
  ...
  const panelTop = ref(window.innerHeight - 270);
...
const requestCourse = async () => {
  ...
  // renderSlopeOverlay()가 별도 Overlay로 그린다 (Base + Overlay 구조). Desktop
  ...
  function addRouteLayer(coordinates, targetMap) {
    ...
    if (targetMap === map.value) {
      ...
      modalPolyline.value.setMap(photoMap.value);
    }
  ...
  // 20m SlopeSection Backend API 기반 경사 Overlay -- Base Trail(drawBaseRoute)
  // 위에 덧그려진다. course 변경 시 이전 Overlay를 반드시 제거한 뒤 새로 그린다(중복 방지).
  // SlopeSection이 존재하지 않는 구간(coverage 한계)은 Base Trail만 계속 보이며,
  ...
  const showTestContent = ref(false);
