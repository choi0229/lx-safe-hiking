...
class TrailGeoJsonServiceTest {
    ...
    void everyFeatureHasExpectedShapeAndProperties() throws Exception {
        ...
        for (JsonNode feature : root.get("features")) {
            ...
            // trailId (t.id) is exposed so the Frontend can resolve the current database's Trail
            // id for a course instead of hardcoding one (trail.id is a surrogate PK).
            // source_feature_index/sequence remain unexposed -- only fields an actual consumer
            // reads belong here.
