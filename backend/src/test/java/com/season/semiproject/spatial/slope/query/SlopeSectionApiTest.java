 ...
 * the real Docker PostgreSQL/PostGIS instance and the actual imported data. The API serves
 * precomputed `slope_section` rows -- this class ensures a build has run before asserting on
 * response content, and that only windowMeters=20 is accepted.
 */
@SpringBootTest
...
class SlopeSectionApiTest {
    ...
    void windowMeters10And30AreNoLongerAcceptedByTheProductionApi() throws Exception {
        ...
        // chk_slope_section_window_m). Accepting 10/30 here would silently reintroduce a
        // mixed-semantics API ("20 -> DB read, 10/30 -> some other computation"), so both must be
        // rejected the same way any other invalid windowMeters value is.
