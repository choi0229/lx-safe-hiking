...
import com.season.semiproject.spatial.trail.ingest.manifest.TrailImportManifest;
...
/**
 * Regression test for the Original(1719) vs API(1706) difference introduced by the exclusion
 * of 13 Features (see trail-import-manifest.json). This difference is INTENTIONAL -- these tests
 * fix it as a known, expected shape rather than treating it as a defect, and separately verify
 * that every Feature the API DOES return still matches its original source Feature exactly
 * (property + geometry parity, precision-aware).
 */
@SpringBootTest
