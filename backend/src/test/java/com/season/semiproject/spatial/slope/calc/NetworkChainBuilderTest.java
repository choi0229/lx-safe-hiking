...
import static com.season.semiproject.spatial.slope.calc.SlopeTestFixtures.segment;

/**
 * Pure unit tests for {@link NetworkChainBuilder} on hand-built graphs -- no DB, no Spring. Covers
 * the linear-chain, junction and cycle cases; only the linear-chain case is exercised by the real
 * dataset today (every TrailNode has degree 1 or 2), but junction/cycle handling must still be
