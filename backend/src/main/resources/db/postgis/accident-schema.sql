...
--                              computed at query time only (com.season.semiproject.spatial.accident.proximity)
...
-- "Legacy UI Display Dataset". Verified: only 5 of its 15 Features carry
...
-- Layer whose ids are not permanent identifiers.
...
-- are the only ones with 0 NULLs across all 42 Features and actual use in spatial
...
-- No GiST index on accident_point.geom -- EXPLAIN ANALYZE at 42 rows showed Seq Scan cost is
-- negligible and no query plan used it. Add only if a future EXPLAIN shows it would actually be
