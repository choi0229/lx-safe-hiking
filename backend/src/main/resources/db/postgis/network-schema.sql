-- Derived Network Layer. Design notes: Feature != Segment, exact-match only (no snapping), and
-- trail_id deliberately NOT duplicated onto these tables (see the comments below).
--
-- Layering recap:
...
-- drop `trail_feature` or `trail` (the Raw Spatial Layer, populated by the trail importer) --
...
--   4) trail import (spatial-import profile), 5) db/postgis/network-schema.sql,
--   6) network build (spatial-network-build profile).
...
-- EXACT coordinate equality only (a near-endpoint tolerance experiment was run but NOT
-- applied -- no snapping, no geometry modification). A degree-1 node is a loose end (trail terminus or a gap boundary);
...
-- TrailSegment
--
-- TrailSegment means EXACTLY ONE THING: a single LineString Part
-- (one ST_Dump() result of a TrailFeature.geom) with its two exact endpoints resolved to
-- TrailNode rows. It is the Derived Network Edge -- it is NOT a GeoJSON Feature ("Feature != Segment") and
-- its geometry is never split further, because no genuine mid-line (non-endpoint) intersection
-- was found in the actual data.
--
