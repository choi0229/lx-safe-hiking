-- Derived Analysis Layer, precomputed and persisted rather than computed per request:
-- SlopeSection is a deterministic function of (Network state, TrailFeature.dn_value,
...
-- tracking between these steps; the order above must be followed manually.
...
--
-- window_m is CHECKed to always be 20: Production only ever consumes the 20m analysis unit; the
-- 10m/30m candidates that were evaluated remain available
