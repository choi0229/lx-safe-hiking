-- PostgreSQL relational schema, translated from the original MySQL DDL. Table/column shapes are
-- kept as close to the original as possible on purpose -- this restores the existing schema, it
...
-- backend/src/main/resources/mapper/mapper-mentordetail.xml but have no CREATE TABLE in the original MySQL DDL.
-- No schema is guessed for them here.
...
-- Original MySQL DDL: `create table user (...)`.
...
-- Original MySQL DDL: `create table mountain (...)`. DOUBLE -> DOUBLE PRECISION.
...
-- Original MySQL DDL: `create table userProfile (...)`. AUTO_INCREMENT -> IDENTITY.
...
-- Original MySQL DDL: `create table usernotification (...)`.
...
-- Original MySQL DDL: `create table userHelpCall (...)`.
...
-- Restored anyway because it exists in the original DDL and the goal here
-- is to reproduce the original DDL, not to prune it.
...
-- Original MySQL DDL: `CREATE TABLE complaint (...)`. DOUBLE -> DOUBLE PRECISION.
-- `updated_at ... ON UPDATE CURRENT_TIMESTAMP` has no PostgreSQL DDL equivalent;
-- reproduced below with the set_updated_at() trigger.
-- No FK on user_id here -- the original DDL never defined one for this table; preserved as-is.
...
-- Original MySQL DDL: `CREATE TABLE processing (...)`.
...
-- Original MySQL DDL: `create table course (...)`.
...
-- the original DDL never declared this FK. This schema restores the original structure verbatim;
-- any redesign is out of scope here.
CREATE TABLE course (
...
-- Original MySQL DDL: `CREATE TABLE course_reviews (...)`.
...
-- Original MySQL DDL: `CREATE TABLE community (...)`. DATETIME -> TIMESTAMP.
...
-- Original MySQL DDL: `create table community_comment (...)`. DATETIME -> TIMESTAMP.
...
-- Original MySQL DDL: `create table community_likes (...)`.
...
-- Original MySQL DDL: `CREATE TABLE path_info (...)`. DATETIME -> TIMESTAMP.
...
-- Original MySQL DDL: `CREATE TABLE tracking_path (...)`.
...
-- runs PostgreSQL 16). The business logic inside
-- each function is kept identical to the original MySQL triggers -- only the MySQL procedural wrapper changed.
-- ============================================================================

-- Original MySQL DDL: `updated_at ... ON UPDATE CURRENT_TIMESTAMP` on complaint (column-level MySQL clause,
...
-- MySQL trigger 1/4: update_complaint_state_after_processing_insert
...
-- MySQL trigger 2/4: update_course_rate_after_insert
...
-- MySQL trigger 3/4: update_course_level_after_insert
...
-- MySQL trigger 4/4: update_path_info_stats
