...
public class SpatialImportRunner implements CommandLineRunner {
    ...
    private int doImport() {
        try {
            ...
            // DB-level failures (e.g. an FK/constraint violation during insert) are rolled back by
            // @Transactional. Logged distinctly from TrailImportException because a row that
            // passed Java-level validation was still rejected by the DB.
