...
/**
 * Spatial relation queries between AccidentPoint (Raw) and TrailSegment (Derived Network).
 * Deliberately stateless and read-only -- no FK, no cached distance, computed fresh on every
 * call via PostGIS (see mapper-spatial-query.xml). `distanceMeters` is a caller-supplied search
 * radius, not a claimed risk/safety radius.
 */
@Service
