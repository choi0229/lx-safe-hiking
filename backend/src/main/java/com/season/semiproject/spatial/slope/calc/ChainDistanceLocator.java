 ...
 * fraction inside that Segment's own span -- used by {@link SlopeSectionBuildService} to cut a
 * SlopeSection's geometry per Segment instead of via a single global fraction over the whole
 * merged chain.
 *
 * Cutting the whole chain's WKT with one {@code ST_LineSubstring(chainGeom, globalFraction)}
 * call is inaccurate: {@code ST_LineSubstring} on a {@code geometry} (not {@code geography}) input
 * measures fraction using the PLANAR (degree-based) length of the input line, while
 * {@code distanceMeters}/window boundaries are computed from GEOGRAPHY length accumulated per
 * Segment. These two length measures are not exactly proportional along a long chain that spans a
 * changing latitude, so the returned geometry's actual length could differ from the declared
 * {@code distanceMeters} by several percent (measured up to ~14% on the real dataset). Cutting one
 * short (median ~3m) Segment at a time keeps the planar/geography mismatch negligible within each
 * individual cut, since a short, nearly straight Segment's local degree-per-meter ratio barely
 * changes across its own length.
 *
